...
export interface RollingNumberProps
  extends BoxProps, StylesApiProps<RollingNumberFactory>, /* @vue-ignore */ ElementProps<'div'> {
  ...
  /**
   * Character used to separate thousands, set to `true` for `,`
   * @default false
   */
  ...
  /**
   * If set, trailing zeros are added to match `decimalScale`
   * @default false
   */
  ...
  /**
   * If set, use tabular (monospace) numbers
   * @default true
   */
  tabularNumbers?: boolean

  /**
   * If set, the root element acts as an `aria-live="polite"` region (`role="status"`) and screen readers announce every value change.
   * When `false`, the root uses `role="img"` so the current value is still accessible but updates are not announced.
   * @default false
   */
