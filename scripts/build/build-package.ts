...
function getInvalidImportSpecifier(relativeFilePath: string) {
  const normalizePath = relativeFilePath.split(path.sep).join('/')

  if (normalizePath.endsWith('/packages.json') || normalizePath.endsWith('.map')) {
    return null
  }

  const withoutExtension = normalizePath.replace(/\.[^/.]+$/, '')
