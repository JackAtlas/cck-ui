...
interface GetElementsSiblingsInput {
  /** Selector used to find parent node, for example '[role="tablist"]', '.c-Text-root' */
  parentSelector: string
  ...
  /** Determines whether arrow keys will be used */
...
export function createScopedKeydownHandler({
  ...
  return (event: KeyboardEvent) => {
    ...
    ).filter((node) => onSameLevel(event.currentTarget as HTMLButtonElement, node, parentSelector))
